let burst_track_hit_running = !1
  ...
  , burst_page_url = (window.burst_enable_cookieless_tracking = burst.options.enable_cookieless_tracking,
window.location.href)
  , burst_completed_goals = []
  , burst_goals_script_url = burst.goals_script_url || "./burst-goals.js";
const burst_import_goals = async()=>{
    (await import(burst_goals_script_url)).default()
}
;
...
let burst_get_cookie = i=>new Promise((e,t)=>{
  ...
  , burst_set_cookie = (t,e)=>{
    ...
    0 < o.length && (o = ";domain=" + o),
    document.cookie = t + "=" + e + ";SameSite=Strict" + r + a + o + ";path=/"
}
...
function burst_enable_cookies() {
    ...
    burst_use_cookies() && burst_uid().then(t=>{
...
let burst_get_time_on_page = ()=>new Promise(t=>{
  ...
  , burst_api_request = o=>new Promise((t,e)=>{
    burst.options.beacon_enabled ? (window.navigator.sendBeacon(burst.beacon_url, JSON.stringify(o.data)),
    t("ok")) : (t = "token=" + Math.random().toString(36).replace(/[^a-z]+/g, "").substring(0, 7),
    wp.apiFetch({
...
async function burst_update_hit(t=!1) {
    ...
    t ? (e.uid = await burst_uid(),
    e.fingerprint = await burst_fingerprint()) : burst_use_cookies() ? e.uid = await burst_uid() : e.fingerprint = await burst_fingerprint(),
    0 < e.time_on_page || !1 !== e.uid) && await burst_api_request({
...
async function burst_track_hit() {
    var t, e;
    burst_initial_track_hit ? burst_update_hit() : (burst_initial_track_hit = !0,
    burst_is_user_agent() || burst_is_do_not_track() || burst_track_hit_running || (burst_track_hit_running = !0,
    e = new CustomEvent("burst_before_track_hit",{
        detail: burst
    }),
    document.dispatchEvent(e),
    t = {
    ...
    burst_use_cookies() ? t.uid = await burst_uid() : t.fingerprint = await burst_fingerprint(),
    e = new CustomEvent("burst_track_hit",{
        detail: t
    }),
    document.dispatchEvent(e),
    e = {
...
function burst_init_events() {
    !burst.options.enable_turbo_mode || "loading" !== document.readyState ? burst_track_hit() : document.addEventListener("load", burst_track_hit),
